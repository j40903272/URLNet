...
### add ###
parser.add_argument('--train.unbias', type=int, default=0, metavar="UNBIAS",
  help="if train with unbias data")
###
...
parser.add_argument('--log.eval_every', type=int, default=50, metavar="EVALEVERY",
  help="evaluate the model every this number of steps (default: 500)")
parser.add_argument('--log.checkpoint_every', type=int, default=1, metavar="CHECKPOINTEVERY",
...
# x -> nparray[int], reverse_dict -> dict[idx, word]
...
pickle.dump(chars_dict, open("tmp1.pkl","wb"))

print("Overall Mal/Ben rate: {}/{}".format(np.sum(labels==1), np.sum(labels==0)))

if FLAGS["train.unbias"] != 0:
#     print('change to unbias')
#     print(chared_id_x[0])
#     print("===============")

    
    if FLAGS["model.emb_mode"] == 1:
        with open("unbias_chared_id_x_train.pkl", "rb") as f:
            chared_id_x = pickle.load(f)
        for e, token_list in enumerate(chared_id_x):
            chared_id_x[e] = [chars_dict.get(i, len(chars_dict)) for i in token_list]
            
    elif FLAGS["model.emb_mode"] == 2:
        with open("unbias_worded_id_x_train.pkl", "rb") as f:
            worded_id_x = pickle.load(f)
        for e, token_list in enumerate(worded_id_x):
            worded_id_x[e] = [words_dict[i] for i in token_list]
    else:
        print("{} emb mode not support with unbias training".format(FLAGS["model.emb_mode"]))
...
train, test = prep_train_test(len(labels), FLAGS["data.dev_pct"])
...
x_test_char = ngramed_id_x[test]

x_train_word = worded_id_x[train]
x_test_word = worded_id_x[test]

x_train_char_seq = chared_id_x[train]
x_test_char_seq = chared_id_x[test]
...
def train_dev_step(x, y, emb_mode, is_train=True):
    ...
    if emb_mode == 1: 
        ...
        feed_dict = {
            ...
            cnn.dropout_keep_prob: p}
    if is_train:
        result, _, step, loss, acc = sess.run([cnn.merged, train_op, global_step, cnn.loss, cnn.accuracy], feed_dict)
    else: 
        result, step, loss, acc = sess.run([cnn.merged, global_step, cnn.loss, cnn.accuracy], feed_dict)

    return result, step, loss, acc
...
with tf.Graph().as_default(): 
    ...
    with sess.as_default():
        ...
        # Save summary
        train_writer = tf.summary.FileWriter(FLAGS["log.output_dir"]+'/train', graph=sess.graph)
        test_writer = tf.summary.FileWriter(FLAGS["log.output_dir"]+'/test', graph=sess.graph)
        ...
        it = tqdm(range(nb_batches), desc="emb_mode {} delimit_mode {} train_size {}".format(FLAGS["model.emb_mode"], FLAGS["data.delimit_mode"], train.shape[0]), ncols=0)
        
        for idx in it:
            batch = next(train_batches)
            x_batch, y_batch = prep_batches(batch)
            train_result, step, loss, acc = train_dev_step(x_batch, y_batch, emb_mode=FLAGS["model.emb_mode"], is_train=True)
            ...
            if step % FLAGS["log.eval_every"] == 0:# or idx == (nb_batches-1): 
                ...
                nb_corrects = 0
                nb_instances = 0
                test_batches =  make_batches(x_test_char_seq, x_test_word, x_test_char, y_test, FLAGS["train.batch_size"], 1, False)#len(x_test_word)
                for test_batch in test_batches:
                    x_test_batch, y_test_batch = prep_batches(test_batch)
                    dev_result, step, batch_dev_loss, batch_dev_acc = train_dev_step(x_test_batch, y_test_batch, emb_mode=FLAGS["model.emb_mode"], is_train=False)
                    with open(FLAGS["log.output_dir"]+"/tmp", "w") as fff:
                        print(dev_result, file=fff)
                    test_writer.add_summary(dev_result, step)
